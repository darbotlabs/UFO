# Add this to your installation script to run the Azure OpenAI configuration helper
Write-ColorOutput "`n📋 Creating Azure OpenAI Configuration Helper..." -Color Cyan
...
$azureHelperPath = Join-Path $projectPath "fix_azure_config.py"
$azureHelperScript | Out-File -FilePath $azureHelperPath -Encoding utf8

Write-ColorOutput "✅ Created Azure OpenAI configuration helper at $azureHelperPath" -Color Green
Write-ColorOutput "   Run this script to fix common Azure OpenAI configuration issues:" -Color Cyan
Write-ColorOutput "   python fix_azure_config.py" -Color White

Write-ColorOutput "`nWould you like to run the Azure OpenAI configuration helper now? (y/n)" -Color Yellow
$runAzureHelper = Read-Host

if ($runAzureHelper -eq "y") {
    Write-ColorOutput "Running Azure OpenAI configuration helper..." -Color Cyan
    python $azureHelperPath
}
else {
    Write-ColorOutput "Skipping Azure OpenAI configuration helper for now." -Color Yellow
    Write-ColorOutput "You can run it later with: python fix_azure_config.py" -Color White
}
